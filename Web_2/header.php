<!doctype html>
<html lang="en">
    <head>
        <title>header-nav</title>
    </head>
    <body>
        <div class="main-log">
            <div class="main-log__width">
                <ul class="main-log__list">

                    <?php
                        session_start();

                        if(!isset($_SESSION['user_id'])) {


                    ?>

                    <li class="main-log__item">
                        <a class="main-log__link" href="login.php">로그인</a>
                    </li>
                    <li class="main-log__item">
                       <a class="main-log__link" href="registerPage.php">회원가입</a>
                    </li>

                    <?php
                        } else {

                        $user_id = $_SESSION['user_id'];
                    ?>

                    <li class="main-log__item">
                       <a class="main-log__link" href="#"><?= $user_id ?>님</a>
                    </li>
                    <li class="main-log__item">
                       <a class="main-log__link" href="OKayPage/logout.php">로그아웃</a>
                    </li>

                    <?php
                        }
                    ?>

                </ul>
            </div>
        </div>
        <header class="main-header">
            <div class="main-header__width">
                <nav class="main-nav__left">
                    <h1 class="main-nav__logo">
                        <a href="index.php">BRAVEGIRLS</a>
                    </h1>
                </nav>
                <div class="toggle__button">
                    <span class="material-icons header__toggle__btn">
                        menu
                    </span>
                </div>
                <div class="main-nav__right">
                    <ul class="main-nav__list">
                        <li class="main-nav__item">
                            <a id="cd_menu" class="main-nav__link">앨범</a>
                        </li>
                        <li class="main-nav__item">
                            <a class="main-nav__link" href="member.php">멤버</a>
                        </li>
                        <li class="main-nav__item">
                            <a class="main-nav__link" href="http://www.bravesound.com/" target="_blank">엔터테이먼트</a>
                        </li>
                        <li class="main-nav__item">
                            <a class="main-nav__link" href="https://cafe.daum.net/BraveGirls0409" target="_blank">팬카페</a>
                        </li>
                    </ul>
                </div>
            </div>
        </header>
        <div class="scroll_graph">
            <div class="scroll_graph_color"></div>
        </div>
    </body>
</html>